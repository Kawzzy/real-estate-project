...
export class CreateApartmentController {
    ...
    @Post()
	async handle(@Body() body: CreateApartmentBodySchema, @Request() request: IRequestBody) {
		const { addressNumber, addressComplement, airConditioner, amenities, areaSize, balcony, bathrooms, bedrooms, builtYear, condominium, condominiumTax, description,
			dinnerRoom, elevator, floors, furniture, garage, gym, heat, imagesIds, kitchen, laundry, livingRoom, ownerId, playground, pool, price, socialSpace, sponsorId, status
